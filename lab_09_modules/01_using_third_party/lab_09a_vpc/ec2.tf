...
locals {
  ...
  num_pub_subnets_otra = length(module.otra_vpc.public_subnets)
}
...
resource "aws_instance" "server_priv_otra" {
  count         = local.num_pub_subnets_otra
  ami           = data.aws_ami.amazon_linux2_kernel_5.id
  instance_type = var.instance_type
  subnet_id     = module.otra_vpc.public_subnets[count.index]
  tags = {
    Name = "vm-priv-otra-${local.name_suffix}-${count.index}"
  }
}